...
export class NewVaultController {
    async create(req, res) {
        try {
            const { privateKey, content, fileName, directory } = req.body

            if (!privateKey || !content || !fileName || !directory) {
                return res.status(400).json({ error: 'Missing required fields' })
            }

            const service = new NewVaultService()
            const filePath = await service.new({ privateKey, content, fileName, directory })
